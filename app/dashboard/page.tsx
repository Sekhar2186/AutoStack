...
export default function CommandCenter() {
  ...
  // Load saved theme & animations preference
  useEffect(() => {
  ...
  // Fetch real user data on mount and when nav changes
  useEffect(() => {
  ...
  const handleGenerate = async (data: { prompt: string; template: string; selectedModel: string }) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update credits immediately in UI (deduct 1), then re-fetch for accuracy
  ...
  const currentTheme = themes[activeTheme] || themes.obsidian;

  return (
    <MotionConfig reducedMotion={animationsEnabled ? "user" : "always"}>
      <main className={`flex h-screen w-full ${currentTheme.bg} text-slate-50 overflow-hidden font-sans transition-colors duration-500 ${activeTheme === "light" ? "theme-light text-slate-900" : ""} ${!animationsEnabled ? "no-animations" : ""}`}>
        {/* Background ambient effects */}
        <div className="fixed inset-0 pointer-events-none overflow-hidden">
          <div className={`absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full ${currentTheme.glow1} blur-[120px] transition-colors duration-500`} />
          <div className={`absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full ${currentTheme.glow2} blur-[120px] transition-colors duration-500`} />
        </div>
                  ...
                  >
                    ...
                    {!generatedApp ? (
                      <div className="flex flex-col items-center justify-center h-[60%] border-2 border-dashed border-white/5 rounded-3xl p-12 text-center">
                        ...
                        <p className="text-sm text-slate-500 max-w-xs mt-2">
                          Generate an app first. Every time you ask the AI to modify or build upon your code, a new version snapshot is saved here.
                        </p>
                      </div>
                    ) : (
